...
CREATE TABLE IF NOT EXISTS pitch (
  ...
  ax DECIMAL COMMENT \'The acceleration of the pitch, in feet per second per second, in x-dimension, determined at y=50 feet.\',
  ay DECIMAL COMMENT \'The acceleration of the pitch, in feet per second per second, in y-dimension, determined at y=50 feet.\',
  az DECIMAL COMMENT \'The acceleration of the pitch, in feet per second per second, in z-dimension, determined at y=50 feet.\',
  ...
  age_bat TINYINT COMMENT 'Batter''s age at the time of the game.',
  age_bat_legacy TINYINT COMMENT 'Legacy format of batter''s age used in historical data.',
  age_pit TINYINT COMMENT 'Pitcher''s age at the time of the game.',
  age_pit_legacy TINYINT COMMENT 'Legacy format of pitcher''s age used in historical data.',
  api_break_x_arm DECIMAL(5,2) COMMENT 'Horizontal break of the pitch in inches from the pitcher''s arm side perspective.',
  api_break_x_batter_in DECIMAL(5,2) COMMENT 'Horizontal break of the pitch in inches from the batter''s perspective.',
  api_break_z_with_gravity DECIMAL(5,2) COMMENT 'Vertical break of the pitch in inches, including the effects of gravity.',
  arm_angle DECIMAL(5,2) COMMENT 'Pitcher''s arm angle at pitch release in degrees.',
  bat_score_diff SMALLINT COMMENT 'Score difference from the batting team''s perspective.',
  bat_win_exp DECIMAL(4,3) COMMENT 'Win expectancy for the batting team at the time of the pitch.',
  batter_days_since_prev_game SMALLINT COMMENT 'Number of days since the batter''s previous game.',
  batter_days_until_next_game SMALLINT COMMENT 'Number of days until the batter''s next game.',
  delta_pitcher_run_exp DECIMAL(4,3) COMMENT 'Change in run expectancy from the pitcher''s perspective as a result of the pitch.',
  estimated_slg_using_speedangle DECIMAL(4,3) COMMENT 'Estimated slugging average based on launch angle and exit velocity.',
  home_score_diff SMALLINT COMMENT 'Score difference from the home team''s perspective.',
  home_win_exp DECIMAL(4,3) COMMENT 'Win expectancy for the home team at the time of the pitch.',
  hyper_speed DECIMAL(5,2) COMMENT 'Enhanced speed metric that accounts for perceived velocity and extension.',
  n_priorpa_thisgame_player_at_bat TINYINT COMMENT 'Number of prior plate appearances for this batter in the current game.',
  n_thruorder_pitcher TINYINT COMMENT 'Number of times through the batting order for the pitcher in the current game.',
  pitcher_days_since_prev_game SMALLINT COMMENT 'Number of days since the pitcher''s previous game.',
  pitcher_days_until_next_game SMALLINT COMMENT 'Number of days until the pitcher''s next game.',
  ...
  pitch_timestamp DATETIME COMMENT \'Timestamp of the pitch from MLB Stats API.\',
...
CREATE TABLE IF NOT EXISTS weather (
  ...
  station CHAR(4) COMMENT 'ICAO code of the weather station',
  valid DATETIME COMMENT 'Timestamp of the weather observation',
  tmpf DECIMAL(5,2) COMMENT 'Temperature in Fahrenheit',
  dwpf DECIMAL(5,2) COMMENT 'Dew point in Fahrenheit',
  relh DECIMAL(5,2) COMMENT 'Relative humidity in percent',
  drct SMALLINT COMMENT 'Wind direction in degrees',
  sknt DECIMAL(5,2) COMMENT 'Wind speed in knots',
  gust DECIMAL(5,2) COMMENT 'Wind gust in knots',
  alti DECIMAL(5,2) COMMENT 'Atmospheric pressure in inches of mercury',
  vsby DECIMAL(5,2) COMMENT 'Visibility in miles',
  wxcodes VARCHAR(30) COMMENT 'Present weather codes (precipitation, etc.)',
  lat DECIMAL(10,6) COMMENT 'Latitude of the weather station',
  lon DECIMAL(10,6) COMMENT 'Longitude of the weather station',
  venue_id INT COMMENT 'Foreign key to the venue table',
  PRIMARY KEY (weather_id),
  UNIQUE INDEX idx_station_valid (station, valid),
  FOREIGN KEY (venue_id) REFERENCES venue(venue_id)
) COMMENT 'Weather data from stations near MLB venues';
"""

_venue = """
CREATE TABLE IF NOT EXISTS venue (
  venue_id INT NOT NULL,
  name VARCHAR(100) COMMENT 'Name of the venue',
  location_city VARCHAR(100) COMMENT 'City where the venue is located',
  location_state VARCHAR(50) COMMENT 'State or province where the venue is located',
  location_address VARCHAR(50) COMMENT 'Address where the venue is located',
  location_latitude DECIMAL(10,6) COMMENT 'Latitude coordinate of the venue',
  location_longitude DECIMAL(10,6) COMMENT 'Longitude coordinate of the venue',
  timezone VARCHAR(50) COMMENT 'Time zone of the venue',
  field_type VARCHAR(50) COMMENT 'Type of playing field (grass, turf, etc.)',
  roof_type VARCHAR(50) COMMENT 'Type of roof (open, closed, retractable, etc.)',
  elevation INT COMMENT 'Height of the venue',
  azimuthAngle INT COMMENT 'the direction the batter is facing',
  PRIMARY KEY (venue_id)
) COMMENT \'Information about MLB venues\';
"""

_game_venue = """
CREATE TABLE IF NOT EXISTS game_venue (
...
CREATE TABLE IF NOT EXISTS game (
  ...
  TYPE VARCHAR(8) COMMENT 'REG if regular season, AS if all star game, otherwise playoff series code',
  PRIMARY KEY (GAME_ID)
...
schemas = [_pitch, _person, _event, _game, _sub, _venue, _game_venue, _weather]
